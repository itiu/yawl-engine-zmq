...
class XSD4YAWLBuilder {
    ...
    protected XSDSchema createYAWLSchema
        ...
        List theElementParticles = createElements(desiredNamesToTypes, definitions);
        ...
        for (Iterator iterator = theElementParticles.iterator(); iterator.hasNext();) {
            XSDParticle elementParticle = (XSDParticle) iterator.next();
...
//        addDefinitions(schema, definitions);
    ...
    private void checkArguments(String[][] desiredNamesToTypes) {
        for (int i = 0; i < desiredNamesToTypes.length; i++) {
            ...
            /**
             * AJH: amended to cater for 4 aray item holding mandatory setting
             */
...
//            }
            if (nameToType.length < 2 || nameToType.length > 4) {
                throw new IllegalArgumentException("Problem: expecting nameToType[] to be of " +
                        "length 2-4. Not[" + nameToType.length + "]");
            }
    ...
    private List createElements(String[][] elementNamesToTypeNames, Set definitions)
            throws YSchemaBuildingException {
        List elementParticles = new ArrayList();
        ...
        for (int i = 0; i < elementNamesToTypeNames.length; i++) {
            ...
            if (null == typeName) {
                ...
                if (elementName.equals("xs:any") && isSchema4Schema) {
                    ...
                    elementParticles.add(any);
                    break;
                } else {
                    //there MUST be an element in the definitions with this name.
                    element = getXSDElementDeclaration(elementName, definitions);
                    if (null == element) {
                        throw new YSchemaBuildingException(
                                "Expected a corresponding element definition for " +
                                elementName);
                    } else if (isAnElementReference(element)) {
                        throw new YSchemaBuildingException(
                                "element = " + XSDUtil.convertToString(element));
                    }// else {
...
//                    }
                ...
                element = _xsdFactory.createXSDElementDeclaration();
            ...
            {
                /**
                 * MF: Add in min/max occurs for mandatory stuff
                 */
                XSDParticle elementParticle = _xsdFactory.createXSDParticle();
                elementParticle.setContent(element);

                if (elemNameToTypeName.length >= 4 && elemNameToTypeName[3].equals(Boolean.toString(true)))
                {
                ...
                elementParticles.add(elementParticle);
            }
            else
            {
                throw new YSchemaBuildingException("Failed to create schema element for " + elementName + ".");
            }
        }
    ...
    private boolean isSchemaForSchemaComponent(String[] elemNameToTypeName) {
        /**
         * AJH: Cater for mandatory flag as set at Schema 7.1
         */
        return elemNameToTypeName.length > 2 ?
