...
/**
 * 
 * @author Lachlan Aldred Date: 28/05/2003 Time: 15:29:33
 * 
...
public class YWorkItem
{
			...
			throws YPersistenceException
	{
		...
		{
			throw new RuntimeException(this + " [when current status is \"" + _status + "\" it cannot be moved to \""
					+ completionStatus + "\"]");
		}
	 ...
	 * timer parameters. The data in the net-level param is a complex type
	 * YTimerType consisting of two elements: 'trigger' (either 'OnEnabled' or
	 * 'OnExecuting'), and 'expiry': a string that may represent a duration
	 * type, a dateTime type, or a long value to be converted to a Date.
	 * 
	 * @param param
	 *            the name of the YTimerType parameter
	 * @param data
	 *            the case or net-level data object
	...
	private boolean unpackTimerParams(String param, YNetData data)
	{
		...
		if (trigger == null)
		{
			_log.warn("Unable to set timer for workitem: " + getIDString() + ". Missing 'trigger' parameter.");
		...
		if (expiry == null)
		{
			_log.warn("Unable to set timer for workitem: " + getIDString() + ". Missing 'expiry' parameter.");
			return false; // no expiry value set            
		}

		if (expiry.startsWith("P"))
		{ // duration types start with P
			try
			{
		...
		{
			_log.warn("Unable to set timer for workitem: " + getIDString() + ". Invalid 'expiry' parameter.");
	...
	public void checkStartTimer(YPersistenceManager pmgr, YNetData data) throws YPersistenceException
	{
		...
		if (_timerParameters != null)
		{
			...
			if (((trigger == YWorkItemTimer.Trigger.OnEnabled) && (_status.equals(statusEnabled)))
					|| ((trigger == YWorkItemTimer.Trigger.OnExecuting) && (_status.equals(statusExecuting))))
			{
				...
				} else
				{
					...
					} else
					{
						...
						if (ticks > 0)
						{
							YTimer.TimeUnit interval = (YTimer.TimeUnit) _timerParameters.get("interval");

							if (interval == null)
								interval = YTimer.TimeUnit.MSEC;
							timer = new YWorkItemTimer(_workItemID.toString(), ticks, interval, (pmgr != null));
							_timerStarted = true;
						}
					}
				}
	...
	public boolean equals(Object other)
	{
		if (this == other)
			return true;
		if (other instanceof YWorkItem)
		{ // instanceof = false if other is null
	...
	public int hashCode()
	{
		return (get_thisID() != null) ? get_thisID().hashCode() : (getWorkItemID() != null) ? getWorkItemID()
	...
	public void setStatusToSuspended(YPersistenceManager pmgr) throws YPersistenceException
	{
		if (hasLiveStatus())
		{
			...
			throw new RuntimeException(this + " [when current status is \"" + _status
					+ "\" it cannot be moved to \"Suspended\".]");
	}
	...
	public JSONObject toJsonObject()
	{
		...
		if (get_specIdentifier() != null)
			arg.put("yawl:specIdentifier", get_specIdentifier());

		if (get_specVersion() != null)
			arg.put("yawl:specVersion", get_specVersion());

		if (get_specUri() != null)
			arg.put("yawl:specUri", get_specUri());

		if (get_status() != null)
			arg.put("yawl:status", get_status());

		if (get_enablementTime() != null)
		{
			arg.put("yawl:enablementTime", get_enablementTime() + "");
			arg.put("yawl:enablementTimeMs", get_enablementTime().getTime() + "");
		}

		if (_dataList != null)
		{
			List list = _dataList.getChildren();
			if (list != null)
			{
	...
	private YLogDataItem getDataLogPredicate(YParameter param, boolean input)
	{
		...
		if (logPredicate != null)
		{
			String predicate = input ? logPredicate.getParsedStartPredicate(param) : logPredicate
					.getParsedCompletionPredicate(param);
			if (predicate != null)
			{
