 ...
 * 
 * 
 * @author Lachlan Aldred Date: 21/01/2004 Time: 11:58:18
 * 
...
public class EngineGatewayImpl implements EngineGateway
{
	...
	{
		return _sessionCache.checkConnection(sessionHandle) ? SUCCESS : failureMessage("Invalid or expired session.");
	}
	...
	/**
	 * Indicates if the engine has encountered some form of persistence failure
	 * in its lifetime.
	 * <P>
	 * 
	...
	/**
	 * Triggers the announcement that engine startup is complete Should only be
	 * called from InterfaceB_EngineBasedServer.init()
	 * 
	...
	public void setActualFilePath(String path)
	{
		...
		if (!path.endsWith("/"))
			path += "/"; // make sure it has ending slash
	...
	public String getAvailableWorkItemIDs(String sessionHandle) throws RemoteException
	{
		...
		if (isFailureMessage(sessionMessage))
	...
	public String getWorkItem(String workItemID, String sessionHandle) throws RemoteException
	{
		...
		return (workItem != null) ? workItem.toXML() : failureMessage("WorkItem with ID '" + workItemID
				+ "' not found.");
	}
	 ...
	 * 
	 * @param specID
	 *            the specification (process definition id)
	 * @param sessionHandle
	 *            the sessionhandle
	...
	public String getSpecificationDataSchema(YSpecificationID specID, String sessionHandle) throws RemoteException
	{
		String sessionMessage = checkSession(sessionHandle);
		if (isFailureMessage(sessionMessage))
			return sessionMessage;
	 ...
	 *            data
	 * @param logPredicate
	 *            a pre-parsed configurable logging string
	 * @param sessionHandle
	 *            sessionhandle
	...
	public String completeWorkItem(String workItemID, String data, String logPredicate, boolean force,
	...
	public String skipWorkItem(String workItemID, String sessionHandle) throws RemoteException
	{
		...
		try
		{
			...
			if (item != null)
			{
				...
				if (child == null)
				{
					throw new YAWLException("Engine failed to skip work item " + item.toString());
				}
				return successMessage(child.toXML());
			}
	...
	/**
	 * Creates a workitem sibling of of the workitemid param. It will only do
	 * this if: 1) the task for workItemID is multi-instance, 2) and if the task
	 * for workItemID is busy 2) and if the task for workItemID allows dynamic
	 * instance creation, 3) and if the number of instances has no yet reached
	 * max instances.
	 * 
	 * @param workItemID
	 *            the id of an existing instance to which to add a new instance
	 * @param paramValueForMICreation
	 *            the data needed to create the new instance
	 * @param sessionHandle
	 *            session handle
	 * @return an xml message indicating result.
	 * @throws RemoteException
	 */
	public String createNewInstance(String workItemID, String paramValueForMICreation, String sessionHandle)
			throws RemoteException
	{
		String sessionMessage = checkSession(sessionHandle);
		if (isFailureMessage(sessionMessage))
	...
	public String getWorkItemsWithIdentifier(String idType, String itemID, String sessionHandle) throws RemoteException
	{
		...
		return describeWorkItems(_engine.getWorkItemRepository().getWorkItemsWithIdentifier(idType, itemID));
	}
	...
	public String getWorkItemsForService(String serviceURI, String sessionHandle) throws RemoteException
	{
		...
		return describeWorkItems(_engine.getWorkItemRepository().getWorkItemsForService(serviceURI));
	}
	 ...
	 * @throws RemoteException
	 */
	public String getTaskInformation(YSpecificationID specificationID, String taskID, String sessionHandle)
			throws RemoteException
	{
		String sessionMessage = checkSession(sessionHandle);
		if (isFailureMessage(sessionMessage))
	 ...
	 *            specID
	 * @param caseParams
	 *            format &lt;data&gt;[InputParam]*&lt;/data&gt; where InputParam
	 ...
	 */
	public String launchCase(YSpecificationID specID, String caseParams, URI caseCompletionURI, String caseID,
			String logDataStr, String sessionHandle)
	{
		String sessionMessage = checkSession(sessionHandle);
		if (isFailureMessage(sessionMessage))
			return sessionMessage;
		...
		try
		{
			...
			return _engine.launchCase(specID, caseParams, caseCompletionURI, caseID, logData, sessionHandle);
		} catch (YAWLException e)
		{
			if (e instanceof YPersistenceException)
			{
	 ...
	 * @return the case id of the launched case, or a diagnostic <failure/> msg.
	 */
	public String launchCase(YSpecificationID specID, String caseParams, URI caseCompletionURI, String logDataStr,
	 ...
	 * 
	 * @param specID
	 *            the process specification id string.
	 * @param sessionHandle
	 *            the sessionhandle
	 * @return a XML message containing a set of caseID elements that are run
	 *         time instances of the process specification with id = specID
	 */
	...
	/**
	 * This method returns a complex XML message containing the state of a
	 * particular case. i.e. every token (identifier) and its position in every
	...
	public String getBuildProperties(String sessionHandle) throws RemoteException
	{
		...
		return (props != null) ? props.toXML() : failureMessage("Unable to retrieve Enigne build properties.");
	}
	...
	/**
	 * Provides an XML list of options for manipulating a work item. Uses the
	 * REST philosophy of showing what you can do with a work item and provides
	 * some links to further manipulate the work item.
	 * 
	 * @param workItemID
	 *            work item id string
	 * @param thisURL
	 *            the url of the engine interface B server (i think).
	 * @param sessionHandle
	 *            the sesssion handle
	 * @return a REST style list of options to change the work item under
	 *         question.
	 */
	public String getWorkItemOptions(String workItemID, String thisURL, String sessionHandle)
	{
		...
		if (workItem != null)
		{
			if (workItem.getStatus().equals(YWorkItemStatus.statusExecuting))
			{
				options.append(
						"<option operation=\"suspend\">"
								...
								+ "<style>post</style>" + "<url>").append(thisURL)
						.append("?action=suspend</url></option>");
				options.append(
						...
						.append(thisURL)
						.append("?action=complete</url>"
								+ "<bodyContent>Any return data needed for this work item.</bodyContent>" + "</option>");
			}
			try
			{
				_engine.checkElegibilityToAddInstances(workItemID);
				options.append(
						...
						.append("?action=createInstance</url>"
								+ "<bodyContent>The data for the new instance.</bodyContent>" + "</option>");
			} catch (YAWLException e)
			{
			...
			{
				options.append(
						"<option operation=\"start\">" + "<documentation>Starts a work item</documentation>"
								+ "<style>post</style>" + "<url>")
						.append(thisURL)
						.append("?action=startOne&amp;user=[userID]</url>"
								+ "<returns>The data provided by the engine for "
								+ "processing the work item.</returns>" + "</option>");
			}
		}
	 ...
	 *            a YAWL schema compliant process specificationStr in its
	 *            entirety, in string format.
	 * @param sessionHandle
	 *            a session handle
	...
	public String loadSpecification(String specificationStr, String sessionHandle)
	{
		...
		if (errorMessages.size() > 0)
		{
			...
			{
				status = message.getStatus().equals(YVerificationMessage.ERROR_STATUS) ? "error" : "warning";
				...
				if (src instanceof YTask)
				{
					...
					if (decomp != null)
					{
						errorMsg.append("<src>").append(decomp.getName() != null ? decomp.getName() : decomp.getID())
								.append("</src>");
					}
				}
				errorMsg.append("<message>").append(message.getMessage()).append("</message>")
						.append("</" + status + ">");
	...
	public String updateAccount(String userName, String password, String doco, String sessionHandle)
	{
		...
		try
		{
			...
			if (success)
				...
				return failureMessage("Unknown account name: " + userName);
		} catch (YPersistenceException ype)
		{
	...
	public String getYAWLServiceDocumentation(String yawlServiceURI, String sessionHandle)
	{
		...
		if (null != service)
		{
			...
			{
				return failureMessage("Yawl service [" + yawlServiceURI + "]" + " has no documentation.");
			}
		} else
		{
			return failureMessage("Yawl service [" + yawlServiceURI + "] " + "not found.");
		}
	}
	 ...
	 * 
	 * @param serviceStr
	 *            an XML message containing the YAWL service details.
	 * @param sessionHandle
	 *            the session handle
	 * @return diagnostic XML message.
	 */
	public String addYAWLService(String serviceStr, String sessionHandle)
	{
		...
		if (null != service)
		{
			...
			if (null == _engine.getRegisteredYawlService(URI))
			{
				if (URI.startsWith("tcp://") || HttpURLValidator.validate(URI).startsWith("<success"))
				{
					try
					{
			...
			{
				return failureMessage("Engine has already registered a service with " + "the same URI ["
						+ service.getURI() + "]");
			}
		} else
		{
			return failureMessage("Failed to parse yawl service from [" + serviceStr + "]");
		}
	}
	...
	public String removeYAWLService(String serviceURI, String sessionHandle)
	{
		...
		return failureMessage("Engine does not contain this YAWL" + " service [ " + serviceURI + " ]");
	...
	public String deleteAccount(String client, String sessionHandle) throws RemoteException
	{
		...
		if (session != null)
		{
			if ((session instanceof YExternalSession) && session.getClient().getUserName().equals(client))
			{
			...
			if (_engine.getExternalClient(client) != null)
			{
				...
				return failureMessage("Unknown client account name: " + client);
		} else
	...
	public String getClientPassword(String userID, String sessionHandle) throws RemoteException
	{
		...
		if (client != null)
		{
			...
			return failureMessage("Unknown account name: " + userID);
	}
	...
	private String getDataForSpecifications(Set<YSpecification> specSet)
	{
		...
		for (YSpecification spec : specSet)
		{
			...
			specs.append(StringUtil.wrap(_engine.getLoadStatus(spec.getSpecificationID()), "status"));
	...
	{
		return _engine.addInterfaceXListener(listenerURI) ? SUCCESS : failureMessage("Add InterfaceX Listener failed.");
	}
	 ...
	 * @throws RemoteException
	 */
	public String getMITaskAttributes(YSpecificationID specificationID, String taskID, String sessionHandle)
			throws RemoteException
	{
		String sessionMessage = checkSession(sessionHandle);
		if (isFailureMessage(sessionMessage))
	...
	public String getResourcingSpecs(YSpecificationID specificationID, String taskID, String sessionHandle)
			throws RemoteException
	{
		String sessionMessage = checkSession(sessionHandle);
		if (isFailureMessage(sessionMessage))
		...
		if (task != null)
			...
			return failureMessage("Unable to get resourcing information for task: " + taskID);
	}
	...
	public String getCaseData(String caseID, String sessionHandle) throws RemoteException
	{
		...
		if (_engine.getCaseID(caseID) != null)
		{
			...
			if (caseData != null)
				...
				return failureMessage("Could not retrieve case data from engine or " + "case data has a null value");
		} else
			return failureMessage("There is no active case with id: " + caseID);
	}
	...
	public String getWorkItemInstanceSummary(String caseID, String sessionHandle) throws RemoteException
	{
		String sessionMessage = checkSession(sessionHandle);
		if (isFailureMessage(sessionMessage))
			return sessionMessage;
	...
	public String rejectAnnouncedEnabledTask(String itemID, String sessionHandle) throws RemoteException
	{
		...
		if (item != null)
		{
			...
			return failureMessage("Unknown worklitem: " + itemID);
	}
