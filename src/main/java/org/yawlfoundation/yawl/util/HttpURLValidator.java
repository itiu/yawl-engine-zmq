...
/**
 * A simple static checker that (1) checks that the url string passed in is a
 * valid URL, and then (2) that the server at the URL is responsive.
 * 
 * Author: Michael Adams Creation Date: 7/05/2009
 */
public class HttpURLValidator
{
			...
			throws MalformedURLException
	{
		...
		{
			if (validate(url).equals("<success/>"))
				return true;
			try
			{
				Thread.sleep(period);
